...
static volatile int   stream_playpos;
...
//  lock_buffer
//============================================================
static __inline int lock_buffer(long offset, long size, void **buffer1, long *length1, void **buffer2, long *length2)
{
...
//  unlock_buffer
//============================================================
static __inline void unlock_buffer(void)
{
	buf_locked--;
	if (buf_locked == 0)
		SDL_UnlockAudio();
...
static __inline void copy_sample_data(short *data, int bytes_to_copy)
{
...
static __inline void update_audio_stream(short *buffer, int samples_this_frame)
{
	// if nothing to do, don't do it
	if (sound_samplerate != 0 && stream_buffer)
	{
		int bytes_this_frame = samples_this_frame * sizeof(short);
		int play_position, write_position, stream_in;
		int orig_write; // used in LOG

		play_position = stream_playpos;

		write_position = stream_playpos + ((sound_samplerate / 60) * sizeof(short));
		...
		if (!stream_in_initialized)
		{
			...
			printf("stream_in = %d, ", (int)stream_in);
			printf("stream_playpos = %d, ", (int)stream_playpos);
			printf("write_position = %d\n", (int)write_position);
...
static void sdl_cleanup_audio()
{
	...
	if (buffer_overflows || buffer_underflows)
		printf("sdl_cleanup_audio:  overflows=%d underflows=%d\n", buffer_overflows, buffer_underflows);
}
...
static int sdl_create_buffers(void)
{
	printf("sdl_create_buffers:  creating stream buffer of %u bytes\n", stream_buffer_size);
...
//		return(0);
	...
	printf("sound_init: Start initialization\n");
	SDL_AudioDriverName(audio_driver, sizeof(audio_driver));
	printf("sound_init: Driver is %s\n", audio_driver);
	initialized_audio = 0;
	...
	printf("sound_init: frequency: %d, channels: %d, samples: %d\n", obtained.freq, obtained.channels, obtained.samples);
	...
	stream_buffer_size = sound_samplerate * sizeof(short) * audio_latency / MAX_AUDIO_LATENCY;
	...
	printf("sound_init: End initialization, frame latency = %d\n", audio_latency);
	return(0);

	// error handling
cant_create_buffers:
cant_start_audio:
	printf("sound_init: Initialization failed. SDL error: %s\n", SDL_GetError());
	return(1);
}
...
{
	if (initialized_audio) {
		printf("sound_kill: closing audio\n");
		SDL_CloseAudio();
		sdl_cleanup_audio();
	}
}
...
{
	if (initialized_audio)
		update_audio_stream((short*)buf, size);
}
